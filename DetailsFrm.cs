...
namespace PrivateIsland_Client
{
    ...
    public partial class DetailsFrm : Form
    {
        ...
        public DetailsFrm(FormStartPosition startWhere)
        {
            InitializeComponent();
        ...
        private void ContinueBtn_Click(object sender, EventArgs e)
        {
            ...
            Settings.Default.psk = PSK;
            Settings.Default.Save();

            CredentialManager.WriteCredential("PrivateIslandClient", Username, Password);
        ...
        private void DetailsFrm_Load(object sender, EventArgs e)
        {
            try
            {
                hostTxt.Text = Settings.Default.host;
                usernameTxt.Text = Settings.Default.username;
                pskTxt.Text = Settings.Default.psk;

                var cred = CredentialManager.ReadCredential("PrivateIslandClient");
                if (cred != null)
                {
                    passwordTxt.Text = cred.Password;

                    var dialogResult = new ConfirmationDialogue("Connect using the previously saved login information?", StartPosition).ShowDialog();
